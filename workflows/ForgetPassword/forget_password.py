...
def execute(context):
    """
    Execute the forget password workflow
    ...
    try:
        # Load config to get database settings and email
        with open('config.json', 'r') as f:
            config = json.load(f)

        # Load the base recording
        with open('recordings/ForgetPassword/forget_password.json', 'r') as f:
        ...
        if success:
            logger.info("=== Forget Password Workflow Completed Successfully ===")
        else:
            logger.error("=== Forget Password Workflow Failed ===")

        return success

    except Exception as e:
        logger.error(f"=== Forget Password Workflow Error: {str(e)} ===")
        try:
            browser.get_driver().save_screenshot("screenshots/forget_password_error.png")
        except:
            pass
        raise
