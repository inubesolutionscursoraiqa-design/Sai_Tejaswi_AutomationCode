...
print("="*80)
print(" RUNNING ALL POSITIVE WORKFLOWS")
print("="*80)

# Define all positive workflows (excluding negative test cases)
positive_workflows = [
...
try:
    ...
    if result.returncode == 0:
        ...
        print(f"\n📁 Check the 'reports/' folder for the latest PDF report")
        print(f"📊 Report includes data from {len(successful_workflows)} successful workflows")
    else:
...
# Final status
if len(successful_workflows) == len(positive_workflows):
    print("🎉 ALL POSITIVE WORKFLOWS COMPLETED SUCCESSFULLY!")
    sys.exit(0)
else:
    print(f"⚠️  {len(failed_workflows)} workflows failed. Check logs for details.")
    sys.exit(1)
